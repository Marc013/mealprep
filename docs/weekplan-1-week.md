---
title: "Weekplan 1 Week"
---

## Uitgangspunten

Dit weekplan gebruikt de vaste shake-momenten uit deze repo en voegt daar per dag 4 maaltijden aan toe:

- 10:15 verse maaltijd
- 13:00 verse maaltijd
- 15:45 mealprep maaltijd
- 18:30 mealprep maaltijd

Trainingsdagen: maandag, dinsdag, donderdag en zaterdag.
{: .text-muted}

<div class="quick-links">
	<a class="quick-link" href="{{ '/' | relative_url }}">Start</a>
	<a class="quick-link" href="{{ '/maaltijden/' | relative_url }}">Alle maaltijden</a>
	<a class="quick-link" href="{{ '/boodschappen' | relative_url }}">Boodschappen</a>
	<a class="quick-link" href="{{ '/batchprep-1-week' | relative_url }}">Batchprep</a>
	<a class="quick-link" href="{{ '/macros' | relative_url }}">Macro-overzicht</a>
</div>

Dagdoelen:

| Dagtype      | Kcal | Eiwitten | Vetten | Koolhydraten |
| :----------- | ---: | -------: | -----: | -----------: |
| Trainingsdag | 2900 |    190 g |   60 g |        400 g |
| Rustdag      | 1880 |    190 g |   80 g |        100 g |

> Alle 15:45 en 18:30 maaltijden zijn zonder broccoli. Op rustdagen is het volume bewust hoog gehouden via champignons, courgette, taugé, sla, komkommer en bloemkoolrijst.

---

## Weekoverzicht

<div class="quick-links">
	<a class="quick-link" href="#dagkaarten">Dagkaarten</a>
	<a class="quick-link" href="#snelle-routes">Snelle routes</a>
	<a class="quick-link" href="#vaste-maaltijden">Vaste maaltijden</a>
	<a class="quick-link" href="#detailoverzicht">Detailoverzicht</a>
</div>

<div id="dagkaarten"></div>

<div class="day-plan-grid">
	<div class="day-plan-card">
		<div class="day-plan-top">
			<span class="day-name">Maandag</span>
			<span class="type type-vers">Training</span>
		</div>
		<div class="day-plan-meal"><strong>07:30</strong> Whey + appel</div>
		<div class="day-plan-meal"><strong>10:15</strong> Power Oats Bowl</div>
		<div class="day-plan-meal"><strong>13:00</strong> Omelet met Brood</div>
		<div class="day-plan-meal"><strong>15:45</strong> Kip Ketjap met Rijst</div>
		<div class="day-plan-meal"><strong>18:30</strong> Stoofpot met Zoete Aardappel</div>
		<div class="day-plan-meal"><strong>21:15</strong> Whey + kiwi</div>
	</div>
	<div class="day-plan-card">
		<div class="day-plan-top">
			<span class="day-name">Dinsdag</span>
			<span class="type type-vers">Training</span>
		</div>
		<div class="day-plan-meal"><strong>07:30</strong> Whey + appel</div>
		<div class="day-plan-meal"><strong>10:15</strong> Power Oats Bowl</div>
		<div class="day-plan-meal"><strong>13:00</strong> Omelet met Brood</div>
		<div class="day-plan-meal"><strong>15:45</strong> Kip Ketjap met Rijst</div>
		<div class="day-plan-meal"><strong>18:30</strong> Stoofpot met Zoete Aardappel</div>
		<div class="day-plan-meal"><strong>21:15</strong> Whey + kiwi</div>
	</div>
	<div class="day-plan-card">
		<div class="day-plan-top">
			<span class="day-name">Woensdag</span>
			<span class="type type-classic">Rust</span>
		</div>
		<div class="day-plan-meal"><strong>07:30</strong> Whey + appel</div>
		<div class="day-plan-meal"><strong>10:15</strong> Volume Zuivelbowl</div>
		<div class="day-plan-meal"><strong>13:00</strong> Grote Omelet-Salade</div>
		<div class="day-plan-meal"><strong>15:45</strong> Kip Ketjap met Bloemkoolrijst</div>
		<div class="day-plan-meal"><strong>18:30</strong> Volume Stoofpot</div>
		<div class="day-plan-meal"><strong>21:15</strong> Whey + kiwi</div>
	</div>
	<div class="day-plan-card">
		<div class="day-plan-top">
			<span class="day-name">Donderdag</span>
			<span class="type type-vers">Training</span>
		</div>
		<div class="day-plan-meal"><strong>07:30</strong> Whey + appel</div>
		<div class="day-plan-meal"><strong>10:15</strong> Power Oats Bowl</div>
		<div class="day-plan-meal"><strong>13:00</strong> Omelet met Brood</div>
		<div class="day-plan-meal"><strong>15:45</strong> Kip Ketjap met Rijst</div>
		<div class="day-plan-meal"><strong>18:30</strong> Stoofpot met Zoete Aardappel</div>
		<div class="day-plan-meal"><strong>21:15</strong> Whey + kiwi</div>
	</div>
	<div class="day-plan-card">
		<div class="day-plan-top">
			<span class="day-name">Vrijdag</span>
			<span class="type type-classic">Rust</span>
		</div>
		<div class="day-plan-meal"><strong>07:30</strong> Whey + appel</div>
		<div class="day-plan-meal"><strong>10:15</strong> Volume Zuivelbowl</div>
		<div class="day-plan-meal"><strong>13:00</strong> Grote Omelet-Salade</div>
		<div class="day-plan-meal"><strong>15:45</strong> Kip Ketjap met Bloemkoolrijst</div>
		<div class="day-plan-meal"><strong>18:30</strong> Volume Stoofpot</div>
		<div class="day-plan-meal"><strong>21:15</strong> Whey + kiwi</div>
	</div>
	<div class="day-plan-card">
		<div class="day-plan-top">
			<span class="day-name">Zaterdag</span>
			<span class="type type-vers">Training</span>
		</div>
		<div class="day-plan-meal"><strong>07:30</strong> Whey + appel</div>
		<div class="day-plan-meal"><strong>10:15</strong> Power Oats Bowl</div>
		<div class="day-plan-meal"><strong>13:00</strong> Omelet met Brood</div>
		<div class="day-plan-meal"><strong>15:45</strong> Kip Ketjap met Rijst</div>
		<div class="day-plan-meal"><strong>18:30</strong> Stoofpot met Zoete Aardappel</div>
		<div class="day-plan-meal"><strong>21:15</strong> Whey + kiwi</div>
		<div class="day-plan-note">Ochtendtraining: 10:15 is je eerste grote herstelmaaltijd.</div>
	</div>
	<div class="day-plan-card">
		<div class="day-plan-top">
			<span class="day-name">Zondag</span>
			<span class="type type-classic">Rust</span>
		</div>
		<div class="day-plan-meal"><strong>07:30</strong> Whey + appel</div>
		<div class="day-plan-meal"><strong>10:15</strong> Volume Zuivelbowl</div>
		<div class="day-plan-meal"><strong>13:00</strong> Grote Omelet-Salade</div>
		<div class="day-plan-meal"><strong>15:45</strong> Kip Ketjap met Bloemkoolrijst</div>
		<div class="day-plan-meal"><strong>18:30</strong> Volume Stoofpot</div>
		<div class="day-plan-meal"><strong>21:15</strong> Whey + kiwi</div>
	</div>
</div>

<div id="snelle-routes"></div>

## Snelle routes

Open op je smartphone direct de juiste maaltijdpagina:

### Vaste momenten

<div class="card-grid">
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-1-whey-shake-ochtend' | relative_url }}">
		<div class="time">07:30</div>
		<div class="name">Whey Shake + Fruit</div>
		<span class="type type-vast">Vast</span>
	</a>
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-6-whey-shake-avond' | relative_url }}">
		<div class="time">21:15</div>
		<div class="name">Whey Shake + Fruit</div>
		<span class="type type-vast">Vast</span>
	</a>
</div>

### Trainingsdag routes

<div class="card-grid">
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-2-trainingsdag-power-oats' | relative_url }}">
		<div class="time">10:15</div>
		<div class="name">Power Oats Bowl</div>
		<span class="type type-vers">Training</span>
	</a>
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-3-trainingsdag-omelet-brood' | relative_url }}">
		<div class="time">13:00</div>
		<div class="name">Omelet met Brood</div>
		<span class="type type-vers">Training</span>
	</a>
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-4-trainingsdag-kip-ketjap-rijst' | relative_url }}">
		<div class="time">15:45</div>
		<div class="name">Kip Ketjap met Rijst</div>
		<span class="type type-mealprep">Training</span>
	</a>
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-5-trainingsdag-stoofpot-zoete-aardappel' | relative_url }}">
		<div class="time">18:30</div>
		<div class="name">Stoofpot met Zoete Aardappel</div>
		<span class="type type-mealprep">Training</span>
	</a>
</div>

### Rustdag routes

<div class="card-grid">
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-2-rustdag-volume-zuivelbowl' | relative_url }}">
		<div class="time">10:15</div>
		<div class="name">Volume Zuivelbowl</div>
		<span class="type type-classic">Rust</span>
	</a>
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-3-rustdag-omelet-salade' | relative_url }}">
		<div class="time">13:00</div>
		<div class="name">Grote Omelet-Salade</div>
		<span class="type type-classic">Rust</span>
	</a>
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-4-rustdag-kip-ketjap-bloemkoolrijst' | relative_url }}">
		<div class="time">15:45</div>
		<div class="name">Kip Ketjap met Bloemkoolrijst</div>
		<span class="type type-classic">Rust</span>
	</a>
	<a class="meal-card" href="{{ '/maaltijden/maaltijd-5-rustdag-volume-stoofpot' | relative_url }}">
		<div class="time">18:30</div>
		<div class="name">Volume Stoofpot</div>
		<span class="type type-classic">Rust</span>
	</a>
</div>

---

<div id="detailoverzicht"></div>

## Vaste maaltijden

### 07:30 — Whey + appel

| Ingrediënt           | Hoeveelheid |
| :------------------- | ----------: |
| Perfect Whey Protein |        50 g |
| Water                |      300 ml |
| Appel                |      1 stuk |

Richtmacro's: 282 kcal, 41 g eiwit, 2,5 g vet, 21 g koolhydraten.

### 21:15 — Whey + kiwi

| Ingrediënt           | Hoeveelheid |
| :------------------- | ----------: |
| Perfect Whey Protein |        50 g |
| Water                |      300 ml |
| Kiwi                 |     2 stuks |

Richtmacro's: 264 kcal, 41 g eiwit, 3 g vet, 18 g koolhydraten.

---

## 10:15 Verse maaltijd

### Trainingsdag — Power oats bowl

| Ingrediënt        | Hoeveelheid |
| :---------------- | ----------: |
| Skyr              |        95 g |
| Halfvolle yoghurt |       200 g |
| Havervlokken      |        38 g |
| Banaan            |        90 g |
| Bosvruchten       |       100 g |
| Rozijnen          |        10 g |
| Honing            |         5 g |
| Kaneel            |  naar smaak |

Bereiding:

1. Doe skyr, yoghurt en havervlokken in een grote kom.
2. Roer de bosvruchten erdoor en laat deze kort ontdooien.
3. Snijd de banaan in plakjes en verdeel erover.
4. Werk af met rozijnen, honing en kaneel.

Richtmacro's: ongeveer 502 kcal, 33,9 g eiwit, 6 g vet, 79,2 g koolhydraten.

### Rustdag — Volume zuivelbowl

| Ingrediënt        | Hoeveelheid |
| :---------------- | ----------: |
| Skyr              |       200 g |
| Halfvolle yoghurt |       200 g |
| Bosvruchten       |       150 g |
| Havervlokken      |         5 g |
| 100% pindakaas    |         5 g |
| Kaneel            |  naar smaak |

Bereiding:

1. Roer skyr en yoghurt los in een grote kom.
2. Voeg bosvruchten en havervlokken toe.
3. Werk af met pindakaas en kaneel.

Richtmacro's: ongeveer 250 kcal, 28 g eiwit, 8 g vet, 24 g koolhydraten.

---

## 13:00 Verse maaltijd

### Trainingsdag — Omelet met brood

| Ingrediënt      | Hoeveelheid |
| :-------------- | ----------: |
| Eieren          |     2 stuks |
| Vloeibaar eiwit |       150 g |
| Tostiham        |        40 g |
| Kaas 45+        |        15 g |
| Champignons     |       150 g |
| Tomaat          |        75 g |
| Komkommer       |        75 g |
| Volkoren brood  |  3 sneetjes |
| Ketchup         |        10 g |

Bereiding:

1. Bak de champignons 4 minuten in een koekenpan.
2. Voeg ei, vloeibaar eiwit, ham en kaas toe en bak tot een stevige omelet.
3. Serveer met brood, tomaat, komkommer en ketchup.

Richtmacro's: ongeveer 572 kcal, 52,2 g eiwit, 18,5 g vet, 54,1 g koolhydraten.

### Rustdag — Grote omelet-salade

| Ingrediënt             | Hoeveelheid |
| :--------------------- | ----------: |
| Eieren                 |     2 stuks |
| Vloeibaar eiwit        |       100 g |
| Kaas 45+               |        25 g |
| Tostiham               |        30 g |
| Champignons            |       150 g |
| Tomaat                 |       150 g |
| Komkommer              |       150 g |
| Gemengde sla           |        75 g |
| Avocado                |        50 g |
| Mayonaise of olijfolie |   optioneel |

Bereiding:

1. Bak champignons, ei, vloeibaar eiwit, ham en kaas tot een stevig eiergerecht.
2. Maak daarnaast een grote salade van tomaat, komkommer, sla en avocado.
3. Gebruik een kleine hoeveelheid mayonaise of olijfolie als dressing.

Richtmacro's: ongeveer 400 kcal, 37 g eiwit, 29 g vet, 15 g koolhydraten.

---

## 15:45 Mealprep maaltijd

### Trainingsdag — Kip ketjap roerbak met rijst

| Ingrediënt        | Hoeveelheid |
| :---------------- | ----------: |
| Kipfilet          |       140 g |
| Witte rijst droog |        80 g |
| Champignons       |       100 g |
| Paprika           |       100 g |
| Ui                |        80 g |
| Courgette         |       150 g |
| Taugé             |        75 g |
| Ketjap manis      |       15 ml |
| Sojasaus          |        5 ml |
| Olijfolie         |        3 ml |
| Knoflook          |   1 teentje |

Bereiding:

1. Kook de rijst gaar volgens de verpakking.
2. Bak de kip op hoog vuur goudbruin.
3. Voeg ui, knoflook, paprika, champignons en courgette toe en roerbak stevig.
4. Voeg taugé, ketjap en sojasaus toe en bak nog kort door.
5. Verdeel met de rijst over mealprep-bakken.

Richtmacro's: ongeveer 660 kcal, 41 g eiwit, 8 g vet, 85 g koolhydraten.

### Rustdag — Kip ketjap roerbak met bloemkoolrijst

| Ingrediënt     | Hoeveelheid |
| :------------- | ----------: |
| Kipfilet       |        75 g |
| Bloemkoolrijst |       175 g |
| Champignons    |       100 g |
| Paprika        |       100 g |
| Ui             |        70 g |
| Courgette      |       100 g |
| Taugé          |        75 g |
| Ketjap manis   |       10 ml |
| Olijfolie      |        5 ml |
| Knoflook       |         5 g |

Bereiding:

1. Bak de kip goudbruin en gaar.
2. Roerbak ui, knoflook, paprika, champignons, courgette en taugé.
3. Voeg ketjap toe en schep alles om.
4. Bak de bloemkoolrijst kort mee in een aparte pan of in de wok.
5. Verdeel over mealprep-bakken.

Richtmacro's: ongeveer 320 kcal, 30,1 g eiwit, 8 g vet, 29,1 g koolhydraten.

---

## 18:30 Mealprep maaltijd

### Trainingsdag — Kip-varkenshaas stoofpot met zoete aardappel

| Ingrediënt             | Hoeveelheid |
| :--------------------- | ----------: |
| Kipfilet               |       100 g |
| Varkenshaas            |       100 g |
| Zoete aardappel        |        80 g |
| Kidneybonen, uitgelekt |        70 g |
| Mais, uitgelekt        |        50 g |
| Champignons            |       100 g |
| Paprika                |       100 g |
| Ui                     |        80 g |
| Passata                |      150 ml |
| Ketjap manis           |       15 ml |
| Olijfolie              |        5 ml |
| Knoflook               |   1 teentje |
| Gerookt paprikapoeder  |        1 tl |
| Komijn                 |        1 tl |

Bereiding:

1. Kook of rooster de zoete aardappel tot gaar.
2. Bak kip en varkenshaas rondom aan.
3. Voeg ui, knoflook, paprika en champignons toe en bak kort mee.
4. Voeg passata, ketjap, bonen, mais en kruiden toe en laat 10 minuten rustig stoven.
5. Meng de zoete aardappel erdoor en verdeel over mealprep-bakken.

Richtmacro's: ongeveer 620 kcal, 61,1 g eiwit, 11,4 g vet, 73,7 g koolhydraten.

### Rustdag — Kip-varkenshaas volume stoofpot

| Ingrediënt             | Hoeveelheid |
| :--------------------- | ----------: |
| Kipfilet               |       100 g |
| Varkenshaas            |        60 g |
| Bloemkoolrijst         |       250 g |
| Champignons            |       150 g |
| Courgette              |       100 g |
| Paprika                |       100 g |
| Ui                     |        60 g |
| Kidneybonen, uitgelekt |        20 g |
| Passata                |       80 ml |
| Avocado                |        30 g |
| Ketjap manis           |       10 ml |
| Olijfolie              |        5 ml |
| Knoflook               |   1 teentje |
| Gerookt paprikapoeder  |        1 tl |
| Komijn                 |        1 tl |

Bereiding:

1. Bak kip en varkenshaas rondom aan.
2. Voeg ui, knoflook, paprika, courgette en champignons toe en bak tot de pan vol maar nog stevig is.
3. Voeg passata, ketjap, bonen en kruiden toe en laat kort inkoken.
4. Bak de bloemkoolrijst apart of schep die op het einde door de pan.
5. Voeg avocado pas na het opwarmen toe voor de beste structuur.

Richtmacro's: ongeveer 330 kcal, 41 g eiwit, 24 g vet, 18 g koolhydraten.

---

## Dagtotalen

De totalen zijn benaderingen op basis van gemiddelde voedingswaarden. In de praktijk zullen merkverschillen en afrondingen iets kunnen afwijken.

| Dagtype      |       Kcal |    Eiwitten |     Vetten | Koolhydraten |
| :----------- | ---------: | ----------: | ---------: | -----------: |
| Trainingsdag | circa 2893 | circa 278 g | circa 49 g |  circa 345 g |
| Rustdag      | circa 1846 | circa 218 g | circa 75 g |  circa 125 g |

Deze praktijkuitkomst blijft binnen je daglimieten: trainingsdag niet boven 2900 kcal en rustdag niet boven 1880 kcal.

## Praktische notities

1. Mealprep de 15:45 en 18:30 maaltijden in twee batches: zondag voor ma-di-wo-do en woensdag voor vr-za-zo.
2. Avocado bij rustmaaltijden bij voorkeur vers toevoegen op de dag zelf.
3. Op zaterdag is de training in de ochtend. Gebruik 10:15 als eerste grote herstelmaaltijd na training.
4. De bijbehorende 7-daagse boodschappenlijst staat op [Weekboodschappen]({{ '/boodschappen' | relative_url }}).
5. De exacte zondag- en woensdagkookverdeling staat op [Batchprep 1 Week]({{ '/batchprep-1-week' | relative_url }}).